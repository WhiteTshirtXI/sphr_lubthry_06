...
if pl == 'x' :
	for i in range(N1) :
		if (tdata[0,i] < 10 and i % 10 == 0) or (i % 100 == 0) or (tdata[0,i] == 1.2) :
		#if (i % 100 == 0) or (tdata[0,i] == 1.2) :
			x = rdata[:,i]
			y = hdata[:,i] + fdata[:,i]
			
			for n in range(len(hdata[:,i])) :
				if fdata[n,i] >= 0 :
					y[n] = 0
			
			strT = str(int(tdata[0,i]))
			if tdata[0,i] == 1.2 :
				strT = '$\overline{t}_0$ = ' + str(tdata[0,i])
			print i
			
			plot(x,y,'.-',label='$\overline{t}$ = '+strT)
